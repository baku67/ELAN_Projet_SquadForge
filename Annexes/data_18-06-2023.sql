...
INSERT INTO `group` (`id`, `leader_id`, `game_id`, `title`, `description`, `nbr_places`, `creation_date`, `restriction_18`, `restriction_lang`, `status`, `img_url`, `restriction_mic`, `candidature_description`, `restriction_img_proof`) VALUES
	(35, 3, 4, 'Team Number One', 'voila on est la team number One on est trop fort rejoins nous', 5, '2023-06-15 23:53:20', 0, 'fr', 'public', NULL, 0, NULL, 0);

-- Listage des données de la table squadforge.group_answer : ~0 rows (environ)
DELETE FROM `group_answer`;

-- Listage des données de la table squadforge.group_question : ~5 rows (environ)
...
-- Listage des données de la table squadforge.group_user : ~0 rows (environ)
...
INSERT INTO `media` (`id`, `title`, `publish_date`, `url`, `status`, `validated`, `user_id`, `game_id`) VALUES
	...
	(42, 'test topic test topic  test topic', '2023-06-15 21:03:27', '646673edac499_1684435949.jpg', 'ouvert', 'validated', 3, 4),
	(43, 'Retest gif upload overwatch gif', '2023-05-26 01:05:36', '646fe9944d7af_1685055892.gif', 'open', 'validated', 3, 1);

-- Listage des données de la table squadforge.media_post : ~18 rows (environ)
...
-- Listage des données de la table squadforge.media_post_like : ~10 rows (environ)
...
-- Listage des données de la table squadforge.media_upvotes : ~10 rows (environ)
...
-- Listage des données de la table squadforge.membre_group : ~1 rows (environ)
...
-- Listage des données de la table squadforge.notification : ~0 rows (environ)
DELETE FROM `notification`;

-- Listage des données de la table squadforge.post_like : ~6 rows (environ)
...
-- Listage des données de la table squadforge.topic_post : ~20 rows (environ)
